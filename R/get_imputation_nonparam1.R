#' Imputation estimator for (beta, sigma) with nonparametrically estimated \eqn{f_{X\mid Z}}
#'
#' @description
...
#'
#' @returns list with \code{$beta_imp_nonparam1}, \code{$sigma_imp_nonparam1}, \code{$root}.
#' @examples
#' library(truncnorm)
#' set.seed(1)
...
#' @export
get_imputation_nonparam1 <- function(y_data,
                                     w_data,
                                     delta_data,
                                     z_data = NULL,
                                     surv_c_vals,
  ...
  ## ---- Full-data score (used after imputation) ----------------------------
  ...
  ## ---- Kernel-imputation scores (with z / without z) ----------------------
  ...
  ## ---- Estimating function: A, B, C, D (kernel imputation) ----------------
  pe_gauss_imp <- function(theta, y_data, w_data, delta_data, z_data,
    ...
    if (!sigma_is_fixed && !z_is_null) {
      ...
      for (i in seq_along(y_data)) {
        val <- val + S_beta_kern1_imp(beta, y_data[i], w_data[i], delta_data[i], z_data[i],
                                      y_data, w_data, delta_data, z_data,
                                      surv_c_vals, sigma)
      }
      return(val)
      ...
      val <- rep(0, 3L)
      for (i in seq_along(y_data)) {
        val <- val + S_beta_kern1_imp_nz(beta, y_data[i], w_data[i], delta_data[i],
                                         y_data, w_data, delta_data,
                                         surv_c_vals, sigma)
      }
      return(val)
      ...
      val <- rep(0, length(theta))
      for (i in seq_along(y_data)) {
        s_i <- S_beta_kern1_imp(beta, y_data[i], w_data[i], delta_data[i], z_data[i],
                                y_data, w_data, delta_data, z_data,
                                surv_c_vals, sigma)
      ...
      val <- rep(0, length(theta))
      for (i in seq_along(y_data)) {
        s_i <- S_beta_kern1_imp_nz(beta, y_data[i], w_data[i], delta_data[i],
                                   y_data, w_data, delta_data,
                                   surv_c_vals, sigma)
        val <- val + s_i[seq_along(theta)] # beta components only
      }
  ...
  theta_hat <- root$x
  if (is.null(sigma_fixed)) {
    sigma_imp_nonparam1 <- as.numeric(tail(theta_hat, 1L))
    beta_imp_nonparam1  <- as.numeric(head(theta_hat, -1L))
  } else {
    sigma_imp_nonparam1 <- as.numeric(sigma_fixed)
    beta_imp_nonparam1  <- as.numeric(theta_hat)
  }

  out <- list(
    beta_imp_nonparam1  = beta_imp_nonparam1,
    sigma_imp_nonparam1 = sigma_imp_nonparam1,
    root = root
  )
  class(out) <- c("imp_nonparam1", class(out))
  out
}
