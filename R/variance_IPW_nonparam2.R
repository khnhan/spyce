#' Plug-in variance for IPW (inverse probability weighting) estimator of (beta, sigma) with nonparametrically estimated \eqn{f_{C\mid Y,Z}}
#'
#' @description
#' Computes the plug-in variance–covariance matrix for the IPW estimator
#' \code{\link{get_IPW_nonparam2}}. Supports four modes matching that estimator:
#' \itemize{
...
#' variance_IPW_nonparam2(beta = c(0.5, 2.5, 0.1, 0.1),
#'                        y_data,
#'                        w_data,
#'                        delta_data,
#'                        z_data,
...
#'                        sigma = 3.5)
#' # CASE B: true = (0, 3, 4)
#' variance_IPW_nonparam2(beta = c(0.5, 2.5),
#'                        y_data,
#'                        w_data,
#'                        delta_data,
...
#' @export
variance_IPW_nonparam2 <- function(beta,
                                   y_data,
                                   w_data,
                                   delta_data,
                                   z_data = NULL,
  ...
  ## ---- Case dispatch (use supplied survival weights) ----------------------
  ...
  if (!sigma_is_fixed && !z_is_null) {
    ...
    S_IPW <- matrix(0, nrow = n, ncol = len.beta + 1L)
    S_ref <- matrix(0, nrow = n, ncol = len.beta + 1L)
    for (i in seq_len(n)) {
      if (delta_data[i]) {
        w <- max(surv_c_vals[i], .Machine$double.eps)
        S_i <- S_beta_f(beta, y = y_data[i], x = w_data[i], z = z_data[i], sigma = sigma)
        S_IPW[i, ] <- S_i / w
        S_ref[i, ] <- S_i
      }
    }
    Ahat <- -stats::cov(S_IPW, S_ref)
    Bhat <-  stats::cov(S_IPW)
    Ainv <- MASS::ginv(Ahat)
    ...
    for (i in seq_len(n)) {
      if (delta_data[i]) {
        w <- max(surv_c_vals[i], .Machine$double.eps)
        S_i <- S_beta_f_nz(beta, y = y_data[i], x = w_data[i], sigma = sigma)
        S_IPW[i, ] <- S_i / w
        S_ref[i, ] <- S_i
      }
    }
    ...
    S_IPW_full <- matrix(0, nrow = n, ncol = len.beta + 1L)
    S_ref_full <- matrix(0, nrow = n, ncol = len.beta + 1L)
    for (i in seq_len(n)) {
      if (delta_data[i]) {
        w <- max(surv_c_vals[i], .Machine$double.eps)
        s_full <- S_beta_f(beta, y = y_data[i], x = w_data[i], z = z_data[i], sigma = sigma_use)
        S_IPW_full[i, ] <- s_full / w
        S_ref_full[i, ] <- s_full
      }
    }
    A_full <- -stats::cov(S_IPW_full, S_ref_full)
    B_full <-  stats::cov(S_IPW_full)
