...
S_beta_imp = function(beta, y, w, delta, alpha1_star,
                      sigma = 4, tau1 = 1){
  #E[I(X>W)S_beta_f(Y,X,Z) | W,Y,Z] / E[I(X>W) | W,Y,Z]
  v_x = 1/(beta[2]^2 / sigma^2 + 1/tau1^2)
  eta_x = v_x * (beta[2] * (y - beta[1]) / sigma^2
...
```{r}
pe_gauss_imp_param1 = function(beta, y_data, w_data, delta_data,
                               sigma = 4, tau1 = 1){
  ...
  n = length(y_data)
  for(i in 1:n){
    val = val + S_beta_imp(beta, y_data[i], w_data[i], delta_data[i],
                           alpha1_MLE,
                           sigma, tau1)
  }
  return(val)
...
```{r}
get_beta_param_imp_param1 = function(k, n, d, 
                                     beta, alpha1, alpha2,
                                     sigma = 4, tau1 = 1, tau2 = 1){
  ...
  tic(k)
  set.seed(k)
  result = nleqslv::nleqslv(beta + rnorm(d+1) * 0.1, pe_gauss_imp_param1, 
                            y_data = y_data,
                            w_data = w_data,
                            delta_data = delta_data,
                            sigma = sigma,
...
# get_beta_param_imp_param1(1, n, d, 
#                        beta, alpha1, alpha2,
...
result_beta = foreach(k = 1:1000,
  ...
  {
    set.seed(k)
    get_beta_param_imp_param1(k, n, d,
                              beta, alpha1, alpha2 = c(3, 0.12),
                              sigma = 4, tau1 = 1, tau2 = 1)
  }

save(result_beta, file = 'Case_imp_param1_low.Rdata')
...
save(result_beta, file = 'Case_imp_param1_mid.Rdata')
...
save(result_beta, file = 'Case_imp_param1_high.Rdata')
...
# Variance estimation
```{r}
estimated_variance_imp_param1 = function(beta, y_data, w_data, delta_data,
                                         sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
                                         eps = 0.001){
  len_beta = length(beta)
  alpha1_MLE = find_alpha1_MLE(beta, y_data, w_data, delta_data,
                               sigma, tau1)
  get_Sbeta_imp = function(beta, y_data, w_data, delta_data,
                           alpha1_star,
                           sigma, tau1){
    # Define x_a
    n = length(y_data)
    ...
    for(i in 1:n){
      Sbeta[i,] = S_beta_imp(beta, y_data[i], w_data[i], delta_data[i],
                             alpha1_star,
                             sigma, tau1)
    }
    return(Sbeta)
  }
  Sbeta_imp = get_Sbeta_imp(beta, y_data, w_data, delta_data, 
                            alpha1_MLE,
                            sigma, tau1)
...
variance = foreach(k = 1:1000,
  ...
  { 
    ...
    w_data = data_k$w_data
    delta_data = data_k$delta_data
    var_k = diag(estimated_variance_imp_param1(
...
save(variance, file = 'Variance_imp_param1_low.Rdata')
...
save(variance, file = 'Variance_imp_param1_mid.Rdata')
...
save(variance, file = 'Variance_imp_param1_high.Rdata')
