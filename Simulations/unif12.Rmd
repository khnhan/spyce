...
b_xz_gauss_unif12 = function(beta, x_a,
                             sigma = 4,
                             tt = 20){
  cc = gauss(tt)
  len = 20
...
```{r}
L_xz_gauss_unif12 = function(beta, x_a,
                                   sigma = 4,
                                   tt = 20){
  cc = gauss(tt)
  len = 20
...
a_gauss_unif12 = function(beta, x_a,
                                sigma = 4,
                                tt = 20){
  MASS::ginv(L_xz_gauss_unif12(beta, x_a,
                                     sigma,
                                     tt)) %*%
    t(b_xz_gauss_unif12(beta, x_a,
                              sigma,
                              tt))
}
```
...
```{r}
S_eff_gauss_unif12 = function(beta, y, w, delta,
                                    x_a, a0,
                                    sigma = 4,
                                    tt = 20){
...
```{r}
pe_gauss_unif12 = function(beta, y_data, w_data, delta_data,
                                 x_a,
                                 sigma = 4,
                                 tt = 20){
  len_beta = length(beta)
  # Define a0
  a0 = a_gauss_unif12(beta, x_a,
                            sigma, tt)
  val = rep(0, len_beta)
  n = length(y_data)
  for(i in 1:n){
    val = val + S_eff_gauss_unif12(beta, y_data[i], w_data[i], delta_data[i],
                                         x_a, a0,
                                         sigma, tt)
  }
  return(val)
...
```{r}
get_beta_param_unif12 = function(k, n, d, 
                                       beta, alpha1, alpha2,
  ...
  tic(k)
  set.seed(k)
  result = nleqslv::nleqslv(beta + rnorm(d+1) * 0.1, pe_gauss_unif12, 
                            y_data = y_data,
                            w_data = w_data,
                            delta_data = delta_data,
                            x_a=x_a,
...
result_beta = foreach(k = 1:1000,
  ...
  {
    set.seed(k)
    get_beta_param_unif12(k, n, d,
                                beta, alpha1, alpha2 = c(3, 0.12),
                                sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20)
  }

save(result_beta, file = 'Case_unif12_low.Rdata')
...
save(result_beta, file = 'Case_unif12_mid.Rdata')
...
save(result_beta, file = 'Case_unif12_high.Rdata')
...
# Variance estimation
```{r}
estimated_variance_unif12 = function(beta, y_data, w_data, delta_data,
                                           sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
                                           eps = 0.001){
  len_beta = length(beta)
  get_Seff_unif = function(beta, y_data, w_data, delta_data,
    ...
    a0 = a_gauss_unif12(beta, x_a,
                              ...
                              tt)
    for(i in 1:n){
      Seff[i,] = S_eff_gauss_unif12(beta, y_data[i], w_data[i], delta_data[i],
                                          x_a, a0,
                                          sigma, tt)
    }
    return(Seff)
  ...
  # Find Sbeta
  get_Sbeta_unif1 = function(beta, y_data, w_data, delta_data,
                       sigma, tau1, tau2){
    n = length(y_data)
    Sbeta = matrix(nrow = n, ncol = len_beta)
    for(i in 1:n){
      Sbeta[i,] = S_beta_unif1(beta, y_data[i], w_data[i], delta_data[i],
                         sigma)
      
    }
    return(Sbeta)
  }
  Sbeta_unif = get_Sbeta_unif1(beta, y_data, w_data, delta_data,
                    sigma, tau1, tau2)
  B = - cov(Seff_unif, Sbeta_unif)
  # Find Sigma2
  Sigma = var(Seff_unif)
  B_inv = MASS::ginv(B)
...
# w_data = data_k$w_data
# delta_data = data_k$delta_data
# estimated_variance_unif12(beta, y_data, w_data, delta_data,
#                                 sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
#                                 eps = 0.001)
```
...
variance = foreach(k = 1:1000,
  ...
  { 
    ...
    w_data = data_k$w_data
    delta_data = data_k$delta_data
    var_k = diag(estimated_variance_unif12(
...
save(variance, file = 'Variance_unif12_low.Rdata')
...
save(variance, file = 'Variance_unif12_mid.Rdata')
...
save(variance, file = 'Variance_unif12_high.Rdata')
