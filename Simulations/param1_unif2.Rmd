...
b_xz_gauss_param1_unif2 = function(beta, x_a,
                                   alpha1_star,
                                   sigma = 4, tau1 = 1,
                                   tt = 20){
  cc = gauss(tt)
...
```{r}
L_xz_gauss_param1_unif2 = function(beta, x_a,
                                   alpha1_star,
                                   sigma = 4, tau1 = 1,
                                   tt = 20){
  cc = gauss(tt)
...
a_gauss_param1_unif2 = function(beta, x_a,
                                alpha1_star,
                                sigma = 4, tau1 = 1,
                                tt = 20){
  MASS::ginv(L_xz_gauss_param1_unif2(beta, x_a,
                                     alpha1_star,
                                     sigma, tau1,
                                     tt)) %*%
    t(b_xz_gauss_param1_unif2(beta, x_a,
                              alpha1_star,
                              sigma, tau1,
                              tt))
}
...
```{r}
S_eff_gauss_param1_unif2 = function(beta, y, w, delta,
                                    x_a, a0,
                                    alpha1_star,
                                    sigma = 4, tau1 = 1,
                                    tt = 20){
...
```{r}
pe_gauss_param1_unif2 = function(beta, y_data, w_data, delta_data,
                                 x_a,
                                 alpha1_star,
                                 sigma = 4, tau1 = 1,
                                 tt = 20){
  len_beta = length(beta)
  # Define a0
  a0 = a_gauss_param1_unif2(beta, x_a,
                            alpha1_star,
                            sigma, tau1, tt)
  val = rep(0, len_beta)
  n = length(y_data)
  for(i in 1:n){
    val = val + S_eff_gauss_param1_unif2(beta, y_data[i], w_data[i], delta_data[i],
                                         x_a, a0,
                                         alpha1_star,
                                         sigma, tau1, tt)
  }
...
pe_gauss_alpha1_MLE = function(beta, y_data, w_data, delta_data,
                               ...
                               tt = 20){
  len_beta = length(beta)
  alpha1_star = find_alpha1_MLE(beta, y_data, w_data, delta_data,
                                sigma, tau1)
  # Define a0
  a0 = a_gauss_param1_unif2(beta, x_a,
                            alpha1_star,
...
```{r}
get_beta_param_param1_unif2 = function(k, n, d, 
                                       beta, alpha1, alpha2,
  ...
  tic(k)
  set.seed(k)
  result = nleqslv::nleqslv(beta + rnorm(d+1) * 0.1, pe_gauss_alpha1_MLE, 
                            y_data = y_data,
                            w_data = w_data,
                            delta_data = delta_data,
                            x_a=x_a,
...
clusterExport(myCluster, list('beta',
                              ...
                              'S_beta_f',
                              'S_beta',
                              'b_xz_gauss_param1_unif2',
                              'L_xz_gauss_param1_unif2',
                              'a_gauss_param1_unif2',
                              'S_eff_gauss_param1_unif2',
                              'pe_gauss_param1_unif2',
                              'data_generating',
                              'get_beta_param_param1_unif2'))
result_beta = foreach(k = 1:1000,
  ...
  {
    set.seed(k)
    get_beta_param_param1_unif2(k, n, d,
                                beta, alpha1, alpha2 = c(3, 0.12),
                                sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20)
  }

save(result_beta, file = 'Case_param1_unif2_low.Rdata')
...
save(result_beta, file = 'Case_param1_unif2_mid.Rdata')
...
save(result_beta, file = 'Case_param1_unif2_high.Rdata')
...
# Variance estimation
```{r}
estimated_variance_param1_unif2 = function(beta, y_data, w_data, delta_data,
                                           sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
                                           eps = 0.001){
  len_beta = length(beta)
  get_Seff_unif2 = function(beta, y_data, w_data, delta_data,
                            alpha1_star,
                            sigma, tau1, tau2,
                            tt, m, eps){
    # Define x_a
    ...
    a0 = a_gauss_param1_unif2(beta, x_a,
                              ...
                              tt)
    for(i in 1:n){
      Seff[i,] = S_eff_gauss_param1_unif2(beta, y_data[i], w_data[i], delta_data[i],
                                          x_a, a0,
                                          alpha1_star,
                                          sigma, tau1, tt)
    }
  ...
  alpha2_MLE = find_alpha2_MLE(y_data, w_data, delta_data,
                               sigma, tau2)
  Seff = get_Seff_unif2(beta, y_data, w_data, delta_data,
                        alpha1_MLE,
                        sigma, tau1, tau2,
                        tt, m, eps)
  ...
  # Find S1
  get_S1 = function(beta, y_data, w_data, delta_data,
                    sigma, tau1, eps){
    log_llhd = function(alpha1_star, y, w, delta){
      v_x = 1/(beta[2]^2 / sigma^2 + 1/tau1^2)
      eta_x = v_x * (beta[2] * (y - beta[1]) / sigma^2
    ...
    S1 = function(alpha1, beta, y, w, delta){
      vec = vector(length = d)
      for(j in 1:d){
        dir = rep(0,d)
        dir[j] = 1
        vec[j] = (log_llhd(alpha1 + eps * dir, y, w, delta) 
                  - log_llhd(alpha1 - eps* dir, y, w, delta))  / (2*eps)
      }
      return(vec)
    }
    alpha1_MLE = find_alpha1_MLE(beta, y_data, w_data, delta_data,
                                 sigma, tau1)
    S1_val = matrix(nrow = n, ncol = d)
    for(i in 1:n){
      S1_val[i,] = S1(alpha1_MLE, beta, y_data[i], w_data[i], delta_data[i])
    }
    return(S1_val)
  }
  S1 = get_S1(beta, y_data, w_data, delta_data,
              sigma, tau1, eps)
  # Find phi1
  get_phi1 = function(S1){
    I1 = var(S1)
    phi1 = S1 %*% MASS::ginv(I1)
    return(phi1)
  }
  phi1 = get_phi1(S1)
  # Find matrices
  A1 = - cov(Seff, S1)
  B = - cov(Seff, Sbeta)
  C = B + A1 %*% (- cov(phi1, Sbeta))
  C_inv = MASS::ginv(C)
  # Find Sigma1
  Sigma1 = var(Seff + phi1 %*% t(A1))
  return(C_inv %*% Sigma1 %*% t(C_inv))
}

# data_k = data_generating(1, n, d, beta, alpha1, alpha2 = c(-1,0.12), sigma, tau1, tau2)
# y_data = data_k$y_data
# w_data = data_k$w_data
# delta_data = data_k$delta_data
# estimated_variance_param1_unif2(beta, y_data, w_data, delta_data,
#                            sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
#                            eps = 0.001)
```
...
clusterExport(myCluster, list('beta',
                              ...
                              'data_generating',
                              'get_beta_param_param1_unif2',
                              'estimated_variance_param1_unif2'))
...
variance = foreach(k = 1:1000,
  ...
  { 
    ...
    w_data = data_k$w_data
    delta_data = data_k$delta_data
    var_k = diag(estimated_variance_param1_unif2(
...
save(variance, file = 'Variance_param1_unif2_low.Rdata')
...
save(variance, file = 'Variance_param1_unif2_mid.Rdata')
...
save(variance, file = 'Variance_param1_unif2_high.Rdata')
