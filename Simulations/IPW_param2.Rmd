...
```{r}
pe_gauss_IPW_param2 = function(beta, y_data, w_data, delta_data,
                               alpha2_star,
                               sigma = 4, tau1 = 1, tau2 = 1){
  ...
  for(i in 1:n){
    if(delta_data[i] == 1){
      val = val + S_beta_f(beta, y_data[i], w_data[i], sigma) / 
        surv_c3(w_data[i], y_data[i], alpha2_star, tau2) 
    }
  }
  return(val)
}
...
```{r}
get_beta_param_IPW_param2 = function(k, n, d, 
                                     beta, alpha1, alpha2,
                                     sigma = 4, tau1 = 1, tau2 = 1){
  ...
  tic(k)
  set.seed(k)
  result = nleqslv::nleqslv(beta + rnorm(d + 1) * 0.1, pe_gauss_IPW_param2, 
                            y_data = y_data,
                            w_data = w_data,
                            delta_data = delta_data,
...
# get_beta_param_IPW_param2(1, n, d, 
...
result_beta = foreach(k = 1:1000,
  ...
  {
    set.seed(k)
    get_beta_param_IPW_param2(k, n, d,
                              beta, alpha1, alpha2 = c(3, 0.12),
                              sigma = 4, tau1 = 1, tau2 = 1)
  }

save(result_beta, file = 'Case_IPW_param2_low.Rdata')
...
save(result_beta, file = 'Case_IPW_param2_mid.Rdata')
...
save(result_beta, file = 'Case_IPW_param2_high.Rdata')
...
# Variance estimation
```{r}
estimated_variance_IPW_param2 = function(beta, y_data, w_data, delta_data,
...
variance = foreach(k = 1:1000,
  ...
  { 
    ...
    w_data = data_k$w_data
    delta_data = data_k$delta_data
    var_k = diag(estimated_variance_IPW_param2(
...
save(variance, file = 'Variance_IPW_param2_low.Rdata')
...
save(variance, file = 'Variance_IPW_param2_mid.Rdata')
...
save(variance, file = 'Variance_IPW_param2_high.Rdata')
