...
```{r}
pe_gauss_MLE_kern1 = function(beta, y_data, w_data, delta_data,
                              surv_c_vals,
                              sigma = 4, tau1 = 1){
  ...
  n = length(y_data)
  for(i in 1:n){
    val = val + S_beta_kern1(beta, y_data[i], w_data[i], delta_data[i],
                             y_data, w_data, delta_data,
                             surv_c_vals,
                             sigma, tau1)
  }
...
```{r}
get_beta_param_MLE_kern1 = function(k, n, d, 
                                    beta, alpha1, alpha2,
                                    sigma = 4, tau1 = 1, tau2 = 1){
  ...
  tic(k)
  set.seed(k)
  result = nleqslv::nleqslv(beta + rnorm(d+1) * 0.1, pe_gauss_MLE_kern1, 
                            y_data = y_data,
                            w_data = w_data,
                            delta_data = delta_data,
...
# get_beta_param_MLE_kern1(1, n, d,
...
result_beta = foreach(k = 1:1000,
  ...
  {
    set.seed(k)
    get_beta_param_MLE_kern1(k, n, d,
                             beta, alpha1, alpha2 = c(3, 0.12),
                             sigma = 4, tau1 = 1, tau2 = 1)
  }

save(result_beta, file = 'Case_MLE_kern1_low.Rdata')
...
save(result_beta, file = 'Case_MLE_kern1_mid.Rdata')
...
save(result_beta, file = 'Case_MLE_kern1_high.Rdata')
...
# Variance estimation
```{r}
estimated_variance_MLE_kern1 = function(beta, y_data, w_data, delta_data,
...
# w_data = data_k$w_data
# delta_data = data_k$delta_data
# estimated_variance_MLE_kern1(beta, y_data, w_data, delta_data,
#                            sigma = 4, tau1 = 1, tau2 = 1, tt = 20, m = 20,
#                            eps = 0.001)
```
...
variance = foreach(k = 1:1000,
  ...
  { 
    ...
    w_data = data_k$w_data
    delta_data = data_k$delta_data
    var_k = diag(estimated_variance_MLE_kern1(
...
save(variance, file = 'Variance_MLE_kern1_low.Rdata')
...
save(variance, file = 'Variance_MLE_kern1_mid.Rdata')
...
save(variance, file = 'Variance_MLE_kern1_high.Rdata')
